...
vals = []
for b in range(batches):
    vals += json.load(open(f'dem_parts/{b:04d}.json'))
missing = sum(v is None for v in vals)
json.dump({'x0': x0, 'y0': y0, 'step': STEP, 'nx': nx, 'ny': ny, 'z': vals}, open('dem.json', 'w'))
